...
function ExpensesTab({ equipment, isAdmin }: { equipment: Equipment; isAdmin: boolean }) {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    const res = await fetch("/api/equipment-expense", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ equipmentId: equipment.id, category: form.category, amount: parseFloat(form.amount), description: form.description, date: form.date }),
    });
    if (res.ok) {
        ...
        <form onSubmit={handleAdd} className="bg-red-50 border border-red-100 rounded-xl p-4 mb-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="text" value={form.category} onChange={e => setForm(p => ({ ...p, category: e.target.value }))} required
                className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-400" placeholder="דלק, חלקים..." />
            </div>
            <div>
          ...
          <div className="flex gap-2">
            <button type="submit" disabled={loading} className="bg-red-400 hover:bg-red-300 disabled:bg-gray-200 text-white text-sm font-medium px-4 py-1.5 rounded-lg transition-colors">{loading ? "שומר..." : "שמור"}</button>
            <button type="button" onClick={() => setShowForm(false)} className="border border-gray-200 text-gray-600 text-sm px-4 py-1.5 rounded-lg hover:bg-gray-50">ביטול</button>
          </div>
        </form>
      )}
