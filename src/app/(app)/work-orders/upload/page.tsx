...
export default function UploadWorkOrderPage() {
	...
	async function handleFileSelect(f: File) {
		...
		const res = await fetch("/api/work-orders/extract", {
			method: "POST",
			body: fd,
		});
		setExtracting(false);
		...
		setForm({
			...
			contractValue:
				data.contractValueBeforeVat?.toString() ||
				data.contractValueWithVat?.toString() ||
				"",
			startDate: data.date || "",
			endDate: "",
			status: "ACTIVE",
	...
	async function handleSubmit(e: React.FormEvent) {
		...
		if (file) {
			const fileRef = ref(
				storage,
				`work-orders/${crypto.randomUUID()}-${file.name}`,
			);
		...
		const body = {
			...
			contractValue: form.contractValue
				? Number.parseFloat(form.contractValue)
				: null,
		...
		<div className="p-4 sm:p-6 lg:p-8 max-w-2xl">
			<div className="mb-6">
				...
				<p className="text-gray-500 text-sm mt-1">
					העלה PDF — המערכת תמלא את הפרטים אוטומטית
				</p>
			</div>
					...
					{!extracting ? (
						...
						<div className="flex flex-col items-center gap-4 py-12">
							<div className="w-14 h-14 rounded-2xl bg-green-50 flex items-center justify-center">
								<svg
									className="w-7 h-7 text-green-600 animate-spin"
									fill="none"
									viewBox="0 0 24 24"
								>
									<circle
					...
					{extractError && (
						<div className="mt-4 bg-red-50 border border-red-100 rounded-xl px-4 py-3 text-red-600 text-sm text-center">
							...
							<button
								...
								className="block mx-auto mt-2 text-red-400 hover:text-red-600 underline text-xs"
							>
								נסה שוב
							</button>
						</div>
