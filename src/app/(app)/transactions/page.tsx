  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="bg-white rounded-2xl border border-gray-100 p-5"><p className="text-xs text-gray-400 mb-1">סה״כ הכנסות</p><p className="text-xl font-bold text-green-600">{formatCurrency(totalIncome)}</p></div>
        <div className="bg-white rounded-2xl border border-gray-100 p-5"><p className="text-xs text-gray-400 mb-1">סה״כ הוצאות</p><p className="text-xl font-bold text-red-600">{formatCurrency(totalExpense)}</p></div>
        <div className="bg-white rounded-2xl border border-gray-100 p-5"><p className="text-xs text-gray-400 mb-1">יתרה</p><p className={`text-xl font-bold ${totalIncome - totalExpense >= 0 ? "text-green-700" : "text-red-700"}`}>{formatCurrency(totalIncome - totalExpense)}</p></div>
      </div>

      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        {transactions.length === 0 ? (
          <div className="p-16 text-center"><p className="text-gray-400 text-sm">אין תנועות עדיין</p></div>
        ) : (
            ...
            <tbody className="divide-y divide-gray-50">
              {transactions.map(t => (
                <tr key={t.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-5 py-4">
                    <Link href={`/sites/${t.workSiteId}`} className="text-sm text-amber-600 hover:underline">{t.workSite?.name || ""}</Link>
                  </td>
