  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        ...
        <Link
          href="/sites/new"
          className="flex items-center gap-2 bg-amber-500 hover:bg-amber-400 text-white font-semibold px-4 py-2.5 rounded-xl transition-colors text-sm"
        >
      ...
      {sites.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-100 p-16 text-center">
          ...
          <p className="text-gray-500 text-sm">אין אתרים עדיין. הוסף את האתר הראשון!</p>
        </div>
      ) : (
          ...
          {sites.map((site) => {
            ...
            return (
              <Link
                key={site.id}
                href={`/sites/${site.id}`}
                className="bg-white rounded-2xl border border-gray-100 p-5 hover:border-amber-200 hover:shadow-md transition-all"
              >
                ...
                {site.clientName && (
                  <p className="text-sm text-gray-600 mb-3">לקוח: {site.clientName}</p>
                )}
                ...
                <div className="flex gap-3 pt-3 border-t border-gray-50">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm font-semibold text-red-600">{formatCurrency(expense)}</p>
                  </div>
                  <div className="flex-1">
                    <p className="text-xs text-gray-400">יתרה</p>
                    <p className={`text-sm font-bold ${balance >= 0 ? "text-green-700" : "text-red-700"}`}>{formatCurrency(balance)}</p>
                  </div>
                </div>
              </Link>
            );
